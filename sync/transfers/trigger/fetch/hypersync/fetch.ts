 ...
 * logs via Envio HyperSync — the fastest log source on that chain. The
 * provider is chain-generic (Base is pre-wired in CHAIN_SETTINGS should it
 * ever be useful as an alternate source). Mirrors the CDP provider's
 * semantics: USDC Transfer events in transactions sent BY the facilitator
 * address (i.e. its settlements).
 *
